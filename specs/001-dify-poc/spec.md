...
- スマートフォン表示域では、音声入力、応答読み上げ、音声関連の操作UIを表示せず、テキストで
  質問と回答閲覧を完了できる
...
- **FR-009**: システムは応答テキストを会話ログに表示し、デスクトップ表示では `speakText` 経由でSpeechSynthesis APIにより読み上げられなければならない
...
- 利用者は採用イベントでスマートフォンを主に利用し、主要な質問・回答導線は
  スマートフォン表示域でテキストのみで検証する。音声入力・応答読み上げは利用しない
