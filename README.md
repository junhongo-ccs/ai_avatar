...
現行プロンプトでは、ナレッジに明記されていない内容を推測して埋めるのではなく、対象外であることを伝えたうえで、必ず `くわしくは、ぜひイベントで社員に聞いてみてください。` という一言につなげるルールを入れています。

- 未記載事項は断定しない
- ただ「分かりません」で終わらせない
- 話題・関心を短く受け止める一言と、社員への案内文の2文で構成する
- あえて答えを絞ることで、学生が社員に直接聞いてみようと思えるきっかけを作る
...
```json
{"face":"joy","messages":["こんにちは！","何か気になることはありますか？"]}
```

- フォールバック形式（`messages` がない場合）:

```json
{"face":"joy","text":"こんにちは！"}
```

- さらにJSONとして解釈できない場合のフォールバック形式:
...
- 回答ごとにアバター表情を切り替え
- `speaking` 状態を表示
- デスクトップ表示ではブラウザのSpeechSynthesisで回答を読み上げ
- スマートフォン表示では音声入力・応答読み上げを使用せず、テキスト表示で会話を継続
...
- スマートフォンブラウザは採用イベントでの主要な利用環境です。主要な質問・回答導線を
  スマートフォン表示域でテキストのみで検証します。参加者同士が知らない学生イベントでの
  利用を想定し、音声入力・応答読み上げ・音声関連の操作UIは提供しません
